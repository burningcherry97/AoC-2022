...
public:
	...
	int most_suitable_size(int to_delete, int current_candidate) {
		for (element* el : members) {
			if (el->type == 1) {
				int size = el->most_suitable_size(to_delete, current_candidate);
				if (size >= to_delete && size < current_candidate) {
...
int main(int argc, char const *argv[])
{
	...
	filesystem->set_size();
	int to_delete = filesystem->size - 40000000;
	cout << filesystem->most_suitable_size(to_delete, filesystem->size) << endl;
